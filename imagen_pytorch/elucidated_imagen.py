...
class ElucidatedImagen(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.random_crop_sizes = cast_tuple(
            random_crop_sizes, num_unets)
        ...
        self.encode_text = partial(
            t5_encode_text, name=text_encoder_name)
        ...
        is_video = any([isinstance(unet, Unet3D)
        ...
        lowres_conditions = tuple(
            map(lambda t: t.lowres_cond, self.unets))
        ...
        self.dynamic_thresholding = cast_tuple(
            dynamic_thresholding, num_unets)
        self.dynamic_thresholding_percentile = dynamic_thresholding_percentile

        # temporal interpolations

        temporal_downsample_factor = cast_tuple(
            temporal_downsample_factor, num_unets)
        ...
        assert tuple(sorted(temporal_downsample_factor, reverse=True)
                     ) == temporal_downsample_factor, 'temporal downsample factor must be in order of descending'
    ...
    def c_out(self, sigma_data, sigma):
        return sigma * sigma_data * (
            sigma_data ** 2 + sigma ** 2) ** -0.5
   ...
   # preconditioned network output
    ...
    def preconditioned_network_forward(
        ...
        **kwargs
        ...
        out = self.c_skip(
            sigma_data, padded_sigma) * noised_images + self.c_out(
            sigma_data, padded_sigma) * net_out
    ...
    def sample_schedule(
        ...
        sigmas = (sigma_max ** inv_rho + steps / (N - 1) *
    ...
    def one_unet_sample(
        ...
        **kwargs
        ...
        resize_kwargs = dict(target_frames=frames) if exists(
            frames) else dict()
        ...
        sigmas = self.sample_schedule(
            hp.num_sample_steps, hp.rho, sigma_min, sigma_max)
        ...
        has_inpainting = exists(
            inpaint_images) and exists(inpaint_masks)
        resample_times = inpaint_resample_times if has_inpainting else 1

        if has_inpainting:
            inpaint_images = self.normalize_img(inpaint_images)
            inpaint_images = self.resize_to(
                inpaint_images, shape[-1],
                **resize_kwargs)
            inpaint_masks = self.resize_to(
                rearrange(inpaint_masks, 'b ... -> b 1 ...').float(),
                shape[-1],
                **resize_kwargs).bool()
        ...
        for ind, (sigma, sigma_next, gamma) in tqdm(
                ...
                total=total_steps, desc='sampling time step',
            ...
            for r in reversed(range(resample_times)):
                ...
                if has_inpainting:
                    images_hat = images_hat * ~inpaint_masks + (
                ...
                denoised_over_sigma = (
                    images_hat - model_output) / sigma_hat

                images_next = images_hat + (
                    sigma_next - sigma_hat) * denoised_over_sigma
                ...
                if has_second_order_correction:
                    ...
                    denoised_prime_over_sigma = (
                        images_next - model_output_next) / sigma_next
    ...
    def sample(
        ...
        if self.is_video and exists(inpaint_images):
            ...
            if inpaint_masks.ndim == 3:
                inpaint_masks = repeat(
                    inpaint_masks, 'b h w -> b f h w', f=video_frames)
        ...
        all_frame_dims = calc_all_frame_dims(
            self.temporal_downsample_factor, video_frames)

        # initializing with an image or video

        init_images = cast_tuple(init_images, num_unets)
        init_images = [
            maybe(self.normalize_img)(init_image)
        ...
        if start_at_unet_number > 1:
            ...
            assert not exists(
                stop_at_unet_number) or start_at_unet_number <= stop_at_unet_number
            assert exists(
                start_image_or_video), 'starting image or video must be supplied if only doing upscaling'
        ...
        for unet_number, unet, channel, image_size, frame_dims, unet_hparam, dynamic_threshold, unet_cond_scale, unet_init_images, unet_skip_steps, unet_sigma_min, unet_sigma_max in tqdm(zip(range(1, num_unets + 1), self.unets, self.sample_channels, self.image_sizes, all_frame_dims, self.hparams, self.dynamic_thresholding, cond_scale, init_images, skip_steps, sigma_min, sigma_max), disable=not use_tqdm):
            ...
            assert not isinstance(
                unet, NullUnet), 'cannot sample from null unet'

            context = self.one_unet_in_gpu(
                unet=unet) if is_cuda and use_one_unet_in_gpu else nullcontext()

            with context:
                lowres_cond_img = lowres_noise_times = None

                shape = (batch_size, channel, *frame_dims,
                ...
                shape = (batch_size, self.channels,
                         *frame_dims, image_size, image_size)
        ...
        pil_images = list(map(lambda img: list(
            map(T.ToPILImage(), img.unbind(dim=0))), outputs))
    ...
    def forward(
        ...
        unet: Union[Unet, Unet3D, NullUnet,
        ...
        **kwargs
        ...
        assert not isinstance(
            unet, NullUnet), 'null unet cannot and should not be trained'
        ...
        batch_size, c, *_, h, w, device, is_video = * \
            images.shape, images.device, (images.ndim == 5)

        frames = images.shape[2] if is_video else None
        all_frame_dims = tuple(
            safe_get_tuple_index(el, 0)
            for el in calc_all_frame_dims(
        ...
        prev_frame_size = all_frame_dims[unet_index -
                                         1] if is_video and not ignore_time and unet_index > 0 else None

        def frames_to_resize_kwargs(frames): return dict(
            target_frames=frames) if exists(frames) else dict()
        ...
        if self.is_video and self.resize_cond_video_frames:
            ...
            temporal_downsample_fn = partial(
                scale_video_time, downsample_scale=downsample_scale)
            kwargs = maybe_transform_dict_key(
                kwargs, 'cond_video_frames', temporal_downsample_fn)
            kwargs = maybe_transform_dict_key(
                kwargs, 'post_cond_video_frames',
                temporal_downsample_fn)
        ...
        if exists(prev_image_size):
            ...
            if self.per_sample_random_aug_noise_level:
                ...
                lowres_aug_time = self.lowres_noise_schedule.sample_random_times(
                    1, device=device)
                lowres_aug_times = repeat(
                    lowres_aug_time, '1 -> b', b=batch_size)

        images = self.resize_to(
            images, target_image_size, **
            frames_to_resize_kwargs(target_frame_size))
        ...
        if exists(random_crop_size):
            ...
            if is_video:
                images, lowres_cond_img = map(lambda t: rearrange(
                    t, '(b f) c h w -> b c f h w', f=frames), (images, lowres_cond_img))
        ...
        self_cond = unet.module.self_cond if isinstance(
            unet, DistributedDataParallel) else unet.self_cond
